...
// Triggers after 3rd successful tune generation.
// Uses the Capacitor App plugin which is already installed.
// No extra package needed — just opens Play Store review dialog via intent.
export async function requestReview() {
  if (!isNative()) return;
  try {
    const { App } = await import('@capacitor/app');
    // Open Play Store listing for in-app review
    // Replace com.tunelab.forzah6 with your actual app ID
    await App.openUrl({ url: 'market://details?id=com.tunelab.forzah6' });
  } catch {}
}

// ── Native share sheet ───────────────────────────────────────────────────────
// Use this instead of navigator.share() or file downloads on Android.
// Opens the real Android share sheet — Discord, WhatsApp, Slack, etc.
export async function nativeShare({ title, text, url }) {
  if (!isNative()) {
    // Web fallback — navigator.share or clipboard
    if (navigator.share) {
      try { await navigator.share({ title, text, url }); return { shared: true }; }
      catch { /* cancelled */ }
    }
...
// ── Share tune as text via native sheet ──────────────────────────────────────
// This is the main share function for tune output
export async function shareTune(tuneText, carName) {
  return nativeShare({
    title: `TuneLab — ${carName}`,
    text:  tuneText,
    url:   'https://ko-fi.com/tunelabs',
  });
}
